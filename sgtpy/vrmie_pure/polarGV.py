...
def Apol(rho, eta, epsa, anij, bnij, cnijk, muad2, npol, sigma3):
    ...
    etav = eta * torch.tensor([0, 1, 2, 3, 4])

    J2Dij = torch.dot(anij + bnij * epsa, etav)
    aux2 = -torch.pi * epsa**2 * sigma3 * npol**2 * muad2**2
    A2 = aux2 * rho * J2Dij

    J3Dijk = torch.dot(cnijk, etav)
    aux3 = - 4 * torch.pi**2 / 3 * epsa**3 * sigma3**2 * npol**3 * muad2**3
...
def dApol_drho(rho, eta, deta, epsa, anij, bnij, cnijk, muad2, npol, sigma3):
    ...
    etavec = torch.tensor([[1., eta, eta**2, eta**3, eta**4],
                      [0., 1., 2*eta, 3*eta**2, 4*eta**3]])

    J2Dij, dJ2Dij = torch.tensordot(etavec, anij + bnij * epsa, axes=((1), (0)))
    aux2 = -torch.pi * epsa**2 * sigma3 * npol**2 * muad2**2
    A2 = aux2 * rho * J2Dij
    dA2 = aux2 * (J2Dij + rho * dJ2Dij * deta)

    J3Dijk, dJ3Dijk = torch.tensordot(etavec, cnijk, axes=((1), (0)))
    aux3 = - 4 * torch.pi**2 / 3 * epsa**3 * sigma3**2 * npol**3 * muad2**3
    A3 = aux3 * rho**2 * J3Dijk
...
def d2Apol_drho(rho, eta, deta, epsa, anij, bnij, cnijk, muad2, npol, sigma3):
    ...
    J2Dij, dJ2Dij, d2J2Dij = torch.tensordot(etavec, anij + bnij * epsa,
                                          axes=((1), (0)))
    ...
    J3Dijk, dJ3Dijk, d2J3Dijk = torch.tensordot(etavec, cnijk, axes=((1), (0)))
    ...
    A = torch.tensor([Apolar, dApolar, d2Apolar])
    return A
