...
def ares(self, rho, temp_aux, Xass0=None):
    ...
    ahs_eval = ahs(eta)
    a1m_eval = self.c*torch.matmul(a1sb_a1, x0_a1)
    suma_a2 = torch.matmul(a1sb_a2, x0_a2)
    ...
    # g1sigma
    suma_g1 = self.c * torch.dot(a1sb_a1[0], x0_a12)
    ...
    suma_g2 = self.c2*torch.dot(a1sb_a2[0],  x0_a22)
    ...
    if self.assoc_bool:
        ...
        a += torch.dot(self.S, (torch.log(Xass) - Xass/2 + 1/2))
    else:
        Xass = None
...
def dares_drho(self, rho, temp_aux, Xass0=None):
    ...
    drho = torch.tensor([1., deta, deta**2])
    ...
    if self.assoc_bool:
        ...
        Dabij = torch.zeros([self.nsites, self.nsites])
        dDabij_drho = torch.zeros([self.nsites, self.nsites])
        ...
        CIJ = rho * torch.tile(Xass**2, (self.nsites, 1)).T * Dabij * self.DIJ
        CIJ[self.diagasso] += 1.
        dXass = dXass_drho(rho, Xass, self.DIJ, Dabij, dDabij_drho, CIJ)
        a[0] += torch.dot(self.S, (torch.log(Xass) - Xass/2 + 1/2))
        a[1] += torch.dot(self.S, (1/Xass - 1/2) * dXass)
    else:
        Xass = None
...
def d2ares_drho(self, rho, temp_aux, Xass0=None):
    ...
    drho = torch.tensor([1., deta, deta**2, deta**3])
    ...
    if self.assoc_bool:
        ...
        Dabij = torch.zeros([self.nsites, self.nsites])
        dDabij_drho = torch.zeros([self.nsites, self.nsites])
        d2Dabij_drho = torch.zeros([self.nsites, self.nsites])
        Dabij[self.indexabij] = Dab
        dDabij_drho[self.indexabij] = dDab
        ...
        a[0] += torch.dot(self.S, (torch.log(Xass) - Xass/2 + 1/2))
        a[1] += torch.dot(self.S, (1/Xass - 1/2) * dXass)
        a[2] += torch.dot(self.S, - (dXass/Xass)**2 + d2Xass * (1/Xass - 1/2))
    else:
        Xass = None
