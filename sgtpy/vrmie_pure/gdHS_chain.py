...
def ki_chain(eta):
    ...
    eta_1 = (1 - eta)
    eta_13 = eta_1**3
    k0 = -torch.log(eta_1) + (42*eta - 39*eta2 + 9*eta3 - 2*eta4)/(6*eta_13)
    k1 = (eta4 + 6*eta2 - 12*eta)/(2*eta_13)
    k2 = -3*eta2/(8*eta_1**2)
    k3 = (-eta4 + 3*eta2 + 3*eta)/(6*eta_13)

    ks = torch.tensor([k0, k1, k2, k3])
    return ks
...
def gdHS(x0_vector, eta):
    ...
    g = torch.exp(torch.dot(x0_vector, ks))
    return g
...
def d2gdHS_drho(x0_vector, eta, deta_drho):
    d2ks = d2ki_chain(eta)
    # xs = np.array([1., x0, x0**2, x0**3])
    d2g = torch.matmul(d2ks, x0_vector)
    d2g[0] = torch.exp(d2g[0])
    d2g[2] += d2g[1]**2
    d2g[2] *= d2g[0]
    d2g[1] *= d2g[0]
    d2g *= deta_drho[:3]
